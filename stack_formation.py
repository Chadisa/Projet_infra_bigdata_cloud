...
try:
    ...
    for instance in instances[0]['Instances']:
        ...
        if (b == True):
            ...
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            stdin, stdout, stderr = ssh.exec_command('wget http://sd-127206.dedibox.fr/hagimont/software/hadoop-2.7.1.tar.gz && wget https://apache.mediamirrors.org/spark/spark-3.0.1/spark-3.0.1-bin-hadoop2.7.tgz')
            fo.write('\n La Commande : wget http://sd-127206.dedibox.fr/hagimont/software/hadoop-2.7.1.tar.gz && wget https://apache.mediamirrors.org/spark/spark-3.0.1/spark-3.0.1-bin-hadoop2.7.tgz  '.encode())
            fo.write('stdout : '.encode())
            fo.write(stdout.read())
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            stdin, stdout, stderr = ssh.exec_command('tar -xvzf hadoop-2.7.1.tar.gz && tar -xvzf spark-3.0.1-bin-hadoop2.7.tgz && rm -r hadoop-2.7.1.tar.gz && rm -r spark-3.0.1-bin-hadoop2.7.tgz')
            fo.write('\n La Commande : tar -xvzf hadoop-2.7.1.tar.gz && tar -xvzf spark-3.0.1-bin-hadoop2.7.tgz && rm -r hadoop-2.7.1.tar.gz && rm -r spark-3.0.1-bin-hadoop2.7.tgz'.encode())
            fo.write('stdout : '.encode())
            fo.write(stdout.read())
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            stdin, stdout, stderr = ssh.exec_command('echo "export HADOOP_HOME=/home/ubuntu/hadoop-2.7.1" >> ~/.profile && source ~/.profile')
            ...
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            stdin, stdout, stderr = ssh.exec_command('echo "export SPARK_HOME=/home/ubuntu/spark-3.0.1-bin-hadoop2.7" >> ~/.profile && source ~/.profile')
            fo.write('\n echo "export SPARK_HOME=/usr/local/spark-3.0.1-bin-hadoop2.7" >> ~/.profile && source ~/.profile '.encode())
            fo.write('stdout : '.encode())
            fo.write(stdout.read())
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            ...
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            #stdin, stdout, stderr = ssh.exec_command('cd spark-2.4.3-bin-hadoop2.7 && sudo ./bin/docker-image-tool.sh -t latest build')
            #fo.write('\n sudo /home/ubuntu/spark-2.4.3-bin-hadoop2.7/bin/docker-image-tool.sh -t latest build '.encode())
            #fo.write('stdout : '.encode())
            #fo.write(stdout.read())
            #fo.write('stderr : '.encode())
            #fo.write(stderr.read())
            stdin, stdout, stderr = ssh.exec_command('wget http://sd-127206.dedibox.fr/hagimont/resources-N7/bigdata/sujet-tp-scale.tgz && tar -xvzf sujet-tp-scale.tgz && rm -r sujet-tp-scale.tgz')
            fo.write('\n wget http://sd-127206.dedibox.fr/hagimont/resources-N7/bigdata/sujet-tp-scale.tgz && tar -xvzf sujet-tp-scale.tgz '.encode())
            fo.write('stdout : '.encode())
            fo.write(stdout.read())
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            stdin, stdout, stderr = ssh.exec_command('mv ./sujet-tp-scale/filesample.txt filesample.txt && mv ./sujet-tp-scale/WordCount.java WordCount.java')
            fo.write('\n mv ./sujet-tp-scale/filesample.txt ./filesample.txt && mv sujet-tp-scale/WordCount.java WordCount.java '.encode())
            fo.write('stdout : '.encode())
            fo.write(stdout.read())
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            stdin, stdout, stderr = ssh.exec_command('rm -r sujet-tp-scale')
            fo.write('\n rm -r sujet-tp-scale '.encode())
            fo.write('stdout : '.encode())
            fo.write(stdout.read())
            fo.write('stderr : '.encode())
            fo.write(stderr.read())
            if (j == 0):
                ...
                fo.write('stderr : '.encode())
                fo.write(stderr.read())
                stdin, stdout, stderr = ssh.exec_command('sudo kubectl create serviceaccount spark && sudo kubectl create clusterrolebinding spark-role --clusterrole=edit --serviceaccount=default:spark --namespace=default')
                fo.write('\n La Commande : sudo kubectl create serviceaccount spark && sudo kubectl create clusterrolebinding spark-role --clusterrole=edit --serviceaccount=default:spark --namespace=default  '.encode())
                fo.write('stdout : '.encode())
                fo.write(stdout.read())
                fo.write('stderr : '.encode())
                fo.write(stderr.read())
